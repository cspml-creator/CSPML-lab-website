/* ===========================================
   main.js — Shared behaviour for sub-pages
   CSPML · IIT Dharwad
   =========================================== */

/* ── Sticky nav: shadow + bg on scroll ── */
...
/* ── Hero canvas — signal waves + network nodes (homepage only) ── */
(function () {
  ...
  function drawWaves() {
    ...
    waves.forEach(w => {
      ...
      ctx.lineWidth = w.lw * 5; ctx.stroke();
      ctx.beginPath();
      for (let x = 0; x <= W; x += 2) {
        const y = H * w.y + Math.sin(x * w.freq + w.phase) * w.amp;
        x === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
  ...
  function drawFog() {
    ...
    fogs.forEach(f => {
      ...
      if (f.x < -pad) f.x = 1+pad; if (f.x > 1+pad) f.x = -pad;
      if (f.y < -pad) f.y = 1+pad; if (f.y > 1+pad) f.y = -pad;
    });
  }
  ...
  function drawNodes() {
    ...
    nodes.forEach((p, i) => {
      ...
      for (let j = i+1; j < nodes.length; j++) {
        const q = nodes[j], d = Math.hypot(px - q.x*W, py - q.y*H), thr = W*0.13;
        if (d < thr) {
          ctx.beginPath(); ctx.moveTo(px, py); ctx.lineTo(q.x*W, q.y*H);
          ctx.strokeStyle = `rgba(0,194,255,${(0.07*(1-d/thr)).toFixed(3)})`;
          ctx.lineWidth = 0.5; ctx.stroke();
        }
      }
      ctx.beginPath(); ctx.arc(px, py, p.r, 0, Math.PI*2);
      ctx.fillStyle = `rgba(0,194,255,${p.a})`; ctx.fill();
      p.x += p.vx; p.y += p.vy;
  ...
  function tick() {
    ...
    drawNodes(); drawWaves(); drawFog(); drawPackets();
